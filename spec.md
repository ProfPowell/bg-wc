...
1. **Name the npm package.** `@profpowell/gl-wc` follows the screen-saver pattern. Confirm scope.
2. **Confetti as a one-shot vs continuous preset.** Confetti can be either burst or loop .
...
4. **OffscreenCanvas availability.**  Use offscreen canvas where we can
5. **Should `motion="force"` exist?** No, never force motion on people who don't want it.
---
