...
export function create({ c2d, getColors }) {
  ...
  function frame(t, params) {
    ...
    // grid lines
    c2d.strokeStyle = rgba(c.fg, 0.07); c2d.lineWidth = 1;
    for (let gy = 0; gy < h; gy += h / 10) { c2d.beginPath(); c2d.moveTo(0, gy); c2d.lineTo(w, gy); c2d.stroke(); }

    // --- streaming area-line chart (top ~55%) ---
    const chH = h * 0.55, pad = w * 0.04;
    const cw = w - pad * 2;
    const samples = 80;
    c2d.beginPath();
    ...
    // area fill
    c2d.lineTo(pad + cw, chH * 0.9); c2d.lineTo(pad, chH * 0.9); c2d.closePath();
    c2d.fillStyle = rgba(c.primary, 0.16); c2d.fill();
    ...
    c2d.strokeStyle = rgba(c.primary, 0.95); c2d.lineWidth = 2; c2d.stroke();

    // --- bouncing bars (bottom-left) ---
    const barTop = chH + h * 0.06, barH = h - barTop - h * 0.04;
    const nb = 14, bw = (w * 0.6) / nb;
    for (let i = 0; i < nb; i++) {
      const v = 0.2 + 0.8 * (0.5 + 0.5 * Math.sin(t * sp * 1.4 + seeds[i % seeds.length] + i * 0.4));
      const bh = v * barH;
    ...
    // --- gauge (bottom-right) ---
    const gx = w * 0.82, gy = barTop + barH * 0.5, gr = Math.min(w * 0.12, barH * 0.5);
    c2d.lineWidth = Math.max(4, gr * 0.18);
    c2d.strokeStyle = rgba(c.fg, 0.12);
    c2d.beginPath(); c2d.arc(gx, gy, gr, Math.PI * 0.75, Math.PI * 2.25); c2d.stroke();
    const val = 0.5 + 0.45 * Math.sin(t * sp * 0.6);
    c2d.strokeStyle = rgba(c.accent, 0.95);
    c2d.beginPath(); c2d.arc(gx, gy, gr, Math.PI * 0.75, Math.PI * 0.75 + val * Math.PI * 1.5); c2d.stroke();
  ...
  return {
    ...
    staticFrame(params) { frame(0.5, params); },
    dispose() {},
  };
}
