...
export function create({ c2d, getColors }) {
  ...
  function frame(t, params) {
    ...
    for (const rt of routes) {
      const A = nodes[rt.a], B = nodes[rt.b];
      const ax = A.x * w, ay = A.y * h, bx = B.x * w, by = B.y * h;
      const mx = (ax + bx) / 2 + (by - ay) * rt.bow;
      const my = (ay + by) / 2 - (bx - ax) * rt.bow;
      c2d.moveTo(ax, ay); c2d.quadraticCurveTo(mx, my, bx, by);
    ...
    for (const [px, py] of packets) { c2d.beginPath(); c2d.arc(px, py, 2.4, 0, Math.PI * 2); c2d.fill(); }

    // nodes (pulsing)
    for (const nd of nodes) {
      const pulse = 0.6 + 0.4 * Math.sin(t * 1.5 + nd.ph);
      c2d.fillStyle = rgba(c.info, 0.9);
      c2d.beginPath(); c2d.arc(nd.x * w, nd.y * h, nd.r, 0, Math.PI * 2); c2d.fill();
      c2d.strokeStyle = rgba(c.info, 0.4 * pulse);
      c2d.lineWidth = 1.2;
      c2d.beginPath(); c2d.arc(nd.x * w, nd.y * h, nd.r + 4 + pulse * 6, 0, Math.PI * 2); c2d.stroke();
    }
  }
  ...
  return {
    ...
    staticFrame(params) { frame(0.3, params); },
    dispose() { nodes = []; routes = []; },
  };
}
