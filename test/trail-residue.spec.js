...
for (const preset of ['matrix', 'mystify']) {
  test(`${preset} trail fade fully clears old ink (no burned-in residue)`, async ({ page }) => {
    await page.goto('/test/new-presets-page.html');
    const r = await page.evaluate(async (name) => {
      const mod = await import(`/src/presets/${name}.js`);
